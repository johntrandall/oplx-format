...
- [x] **VERIFIED 2026-05-27 (VM cross-check)** — Hand-modified filter bplist round-trips cleanly. Test: extracted the `Due Soon` filter (`status == 1`) from `examples/with-baseline.oplx`, decoded the base64 → NSKeyedArchiver dict via `plistlib.loads`, mutated `plist['$objects'][15]` from `1` to `99`, re-serialized via `plistlib.dumps(fmt=FMT_BINARY)`, base64-encoded, substituted back into `<filter name="Due Soon">…</filter>`, re-zipped the bundle, opened in OmniPlan, saved, and re-inspected — the post-save filter still has `objs[15] == 99`. All 12 sibling filters preserved (no fallout from the mutation). Recipe documented in `toc-xml.md` § "`<filter>` blocks".

  **Constraints discovered during the probe:**
  1. The exact object index of the constant value depends on the predicate's predicate type (simple comparison vs. compound vs. function expression). Use `plistlib.loads` + dict inspection to locate it, never hard-code an index.
  2. `plistlib.dumps(fmt=FMT_BINARY)` produces bytes slightly smaller than OmniPlan's original (different serializer ordering). OmniPlan accepts the differently-sized bplist without complaint.
