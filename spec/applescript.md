...
| `add work time` | `add work time <schedule> from "HH:MM" to "HH:MM" [weekday N \| on <date>]` | **Verified broken 2026-05-27 (VM cross-check)** — see broken-commands table below. (Earlier "weekday form Verified" claim was wrong and is retracted.) |
| `subtract work time` | (same params as `add work time`) | **Verified broken 2026-05-27** — see broken-commands table below. |
...
| `add work time` / `subtract work time` (all forms) | **Verified broken 2026-05-27 (VM cross-check)** — both commands fail `-10000` "AppleEvent handler failed" for every parameter form tested: `weekday N` integer, mutated `current date` AppleScript date object, `date "Wednesday, June 17, 2026 at 12:00:00 AM"`, `date "June 17, 2026"`, `date "6/17/2026"`, and ISO-string. Failure is symmetric across project-level and per-resource schedules, and across hand-built fixtures and OmniPlan-saved bundles. The prior claim that `weekday N` worked is retracted — it fails too. Alternate access (`set duration of week day schedule N`) errors `-10008`/`-1700`. | Hand-edit the resource `<schedule>` XML block directly. Per-resource override emitting a full day's `<schedule-day>` is Verified; calendar-day exceptions via `<calendar>` events are likewise XML-only. |
...
| `export ... as "MSPDI"` | **Verified broken 2026-05-27 (VM cross-check)**: error `(6)` "could not be exported as '<filename-root>'" regardless of extension (tested `.mspdi`, `.xml`, `.mpp` — all fail). Same fixture exports cleanly via `as "MPP"` to those same extensions, so the failure is per-format, not per-extension. | Use `as "MPP"` for MS Project interop (produces a real `.mpp` binary, OLE2 compound-file magic `d0cf11e0`, ~232KB for a small fixture). |
| `export ... as "HTML Task List"` (and other HTML variants) | **Verified broken 2026-05-27 (VM cross-check)** in OmniPlan 4.10.2 regardless of `OPHTMLTemplate` state (unset, deleted, or pointed at bundled `.../HTMLTemplates/Bright`). Dialog: "The HTML template you are trying to use is out of date. The customized HTML template at <PATH> is from an older version of OmniPlan and may no longer work correctly." • "Use Default Template" → CRASHES OmniPlan (Exception 0x00000002, doc closed, `-609`). • "Try Customized Template" → 0-byte file, no crash. | Use `as "CSV"` instead (CSV export is working). |
| `fix violation with action "..."` | **Verified 2026-05-27 (VM cross-check)** with 3 violation types exercised. Enumerate valid strings via `actions of <violation>`. Examples: hammock-no-prereqs → `setToTask`; manual-date-earlier-than-constraint → `automatic` \| `moveStart` \| `removeConstraint`; dependency-loop → `remove` \| `removeOther-<N>`. **Invalid action strings are silently ignored** — no error, no fix. | Always enumerate via `actions of <violation>` first; never hard-code action names without checking. |
